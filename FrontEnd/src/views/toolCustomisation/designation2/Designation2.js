...
const Project = () => {
  ...
  const handleSwitchChange = async (
    ...
    if (!checked) {
      ...
      const elapsedTimeInHours = (elapsedTimeInSeconds / 3600).toFixed(2)
      setSpentTime(elapsedTimeInHours)
  ...
  const handleSubmit1 = async (taskStatus, userDesignation) => {
    try {
      let newEvent
      let data
  ...
  const handleSubmit = async () => {
    try {
      if (!validateForm()) {
        ...
        let timeSpentValue = 0 // Default value for Time_spent

        if (userDesignation !== 2) {
          try {
            const response = await axios.get(
              `${USER_API_ENDPOINT}getTesterTime/${activityID2}/${userID}`,
            )

            const requiredTime = response.data.data
            timeSpentValue = requiredTime // Use requiredTime for Time_spent
          } catch (error) {
            console.error('Error fetching timer data:', error)
          }
          ...
          const data = await axios.post(`${USER_API_ENDPOINT}addTimeTracking`, {
            ...
            Time_remaining: (parseFloat(activityTime) - parseFloat(timeSpentValue)).toFixed(2),
            Start_Date: dateTime,
  ...
  // const filterByStartedStatus = (excludedStatuses) => {
  //   return projectPlanning_to_assign.project_planning_moduleDetails
  //     .map((module) => ({
  //       ...module,
  //       projectPlanning_module_tasks_details: module.projectPlanning_module_tasks_details
  ...
  // const combineAndSortDescriptions = () => {
  //   const allDescriptions = [
  ...
  //   return allDescriptions.sort((a, b) => new Date(b.dateTime) - new Date(a.dateTime))
  // }
                ...
                <div id="input">
                  <div className="div1">
                    ...
                    <input
                      ...
                      readOnly
                      style={{
                        width: userDesignation === 2 ? '440px' : '100%',
                ...
                <CForm>
                  <div id="CKeditor1">
                    ...
                    <CKEditor
                      ...
                      config={{
                        ckfinder: {
                          uploadUrl: `${USER_API_ENDPOINT}addActivityFile`,
                        },
                        toolbar: [
                          'heading',
                          '|',
                  ...
                  <div className="time-log-report1">
                    ...
                    <table
                      className={`data-table${
                        userDesignation === 1 ? '3X' : userDesignation !== 2 ? 'X' : '2X'
                      }`}
                    >
                      <thead>
                        <tr>
                          ...
                          {userDesignation !== 1 && <th>Timer</th>}
                          {userDesignation !== 2 && <th>Assign for testing</th>}
                          {userDesignation === 1 ? (
                            <th>Time Tracking Logs</th>
                          ) : (
                            <th>Time Tracking</th>
                          )}
                        </tr>
                                      ...
                                      <CFormSwitch
                                        onChange={(event) =>
                                          handleSwitchChange(
                                            ...
                                            activities.project_modules_activities,
                                            activities.projectPlanning.project_name,
                                          )
                                        }
                                        className={`form-switch-lg ${
                                          timer.isChecked ? 'text-primary' : ''
                                        }`}
                                        style={{
                                          cursor: 'pointer',
                                          textAlign: 'center',
                                ...
                                {userDesignation !== 1 ? (
                                  <td>
                                    <CButton
                                      ...
                                      id="CK1"
                                      onClick={() =>
                                        handleClick2(activities.id, activities.assigned_hrs)
                                      }
                                    >
                                      +
                                    </CButton>
